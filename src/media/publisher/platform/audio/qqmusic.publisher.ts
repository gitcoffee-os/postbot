...
export const getQqMusicAlbumPublishUrl = (albumId) => {
    if (!albumId) {
        // TODO
        albumId = '';
    }
    const publishUrl = `https://mp.tencentmusic.com/content/program/create?albumId=${albumId}&status=-1`;
    console.log('publishUrl', publishUrl);
    return publishUrl;
}

export const qqmusicAudioPublisher = async (data) => {
    console.log('qqmusicAudioPublisher data', data);
    ...
    const formElement = {
        ...
        title: 'input[type="text"][placeholder="请输入作品名称"]',
