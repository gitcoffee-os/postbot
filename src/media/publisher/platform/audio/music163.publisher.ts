...
export const getVoicelistPublishUrl = (voicelistId) => {
    if (!voicelistId) {
        // TODO
        voicelistId = '';
    }
    const publishUrl = `https://music.163.com/st/ncreator/upload?userType=3&voicelistId=${voicelistId}`;
    console.log('publishUrl', publishUrl);
    return publishUrl;
}

export const music163AudioPublisher = async (data) => {
    console.log('music163AudioPublisher data', data);
    ...
    const formElement = {
        audioUpload: 'input.uploader[type="file"]',
        title: 'span.name-input input[type="text"]',
