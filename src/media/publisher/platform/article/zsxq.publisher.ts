...
export const getGroupArticlePublishUrl = (groupId) => {
    ...
    const publishUrl = `https://wx.zsxq.com/article?groupId=${group.groupId}`;
    console.log('publishUrl', publishUrl);
    return publishUrl;
}

export const zsxqArticlePublisher = async (data) => {
    console.log('zsxqArticlePublisher data', data);
    ...
    const formElement = {
        ...
        submitButtons: 'div.operation-btns',
    ...
    const autoFillContent = async() => {
        ...
        if (titleInput) {
            (titleInput as HTMLInputElement).value = contentData?.title?.slice(0, fromRule.title.max) || '';
        ...
        // }
        // titleInput.click();
        // titleInput.dispatchEvent(new Event('click', { bubbles: true }));
        // await sleep(2000)
    ...
    const getSaveDraftButton = () => {
        const buttons = document.querySelectorAll(formElement.submitButtons);
